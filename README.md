...
5. 组件中的变量传给插槽：
  * 把需要传给插槽的变量绑定到`slot`上。
    ...
    ```
  * 在使用插槽的时候，指定名称后，加上一个名称，这个名称可以随便取，那么绑定到`slot`上所有的属性都可以通过这个名称获得。
    ```html
